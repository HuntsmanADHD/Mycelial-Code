...
# ================================================================`;
  ...
  generateStringFunctions() {
    ...
    lines.push(`    pop rcx`);
    lines.push(``);
    lines.push(`    # Detect type: if value < 0x1000000 (16MB), treat as integer; else string`);
    lines.push(`    # This threshold handles large byte counts while being below typical heap addresses`);
    lines.push(`    mov rdx, 0x1000000`);
    ...
    // char_code_at(str, index) -> byte value (not a string!)
    lines.push(`# ================================================================`);
    lines.push(`# builtin_char_code_at(str: *u8, index: u64) -> u8`);
    lines.push(`# Returns the byte value at the given index (NOT a string)`);
    lines.push(`# Arguments: rdi = str, rsi = index`);
    lines.push(`# Returns: rax = byte value (0 if null or out of bounds)`);
    lines.push(`# ================================================================`);
    lines.push(`builtin_char_code_at:`);
    lines.push(`    push rbp`);
    lines.push(`    mov rbp, rsp`);
    lines.push(``);
    lines.push(`    # Check if string is null`);
    lines.push(`    test rdi, rdi`);
    lines.push(`    jz .char_code_at_zero`);
    lines.push(``);
    lines.push(`    # Load the byte at index (no bounds check for performance)`);
    lines.push(`    movzx rax, byte ptr [rdi + rsi]`);
    lines.push(`    pop rbp`);
    lines.push(`    ret`);
    lines.push(``);
    lines.push(`.char_code_at_zero:`);
    lines.push(`    xor eax, eax              # return 0`);
    lines.push(`    pop rbp`);
    lines.push(`    ret`);
    lines.push(``);
  ...
  generateIOFunctions() {
    ...
    // write_bytes(path, vec<u8>) -> success
    // This function writes a vec<u8> to a file, packing the bytes properly
    // (each element in vec<u8> is stored as 8 bytes, we extract just the low byte)
    lines.push(`# ================================================================`);
    lines.push(`# builtin_write_bytes(path: *u8, bytes: *vec<u8>) -> i64`);
    lines.push(`# Writes vec<u8> contents to file, packing bytes properly`);
    lines.push(`# Arguments: rdi = file path, rsi = vec<u8> pointer`);
    lines.push(`# Returns: rax = 1 on success, 0 on error`);
    lines.push(`# ================================================================`);
    lines.push(`builtin_write_bytes:`);
    lines.push(`    push rbp`);
    lines.push(`    mov rbp, rsp`);
    lines.push(`    push r12                  # file descriptor`);
    lines.push(`    push r13                  # packed buffer pointer`);
    lines.push(`    push r14                  # byte count (vec len)`);
    lines.push(`    push r15                  # path pointer`);
    lines.push(`    push rbx                  # vec pointer`);
    lines.push(``);
    lines.push(`    mov r15, rdi              # save path pointer`);
    lines.push(`    mov rbx, rsi              # save vec pointer`);
    lines.push(``);
    lines.push(`    # Get vec length`);
    lines.push(`    mov r14, [rbx + 8]        # vec.len`);
    lines.push(``);
    lines.push(`    # Allocate packed buffer`);
    lines.push(`    mov rdi, r14              # size = vec.len`);
    lines.push(`    call builtin_heap_alloc`);
    lines.push(`    mov r13, rax              # save packed buffer pointer`);
    lines.push(``);
    lines.push(`    # Pack bytes: copy low byte from each 8-byte slot`);
    lines.push(`    mov rcx, r14              # counter = len`);
    lines.push(`    test rcx, rcx`);
    lines.push(`    jz .write_bytes_open      # skip if empty`);
    lines.push(`    mov rsi, [rbx + 0]        # vec.ptr (source)`);
    lines.push(`    mov rdi, r13              # packed buffer (dest)`);
    lines.push(`.write_bytes_pack_loop:`);
    lines.push(`    mov al, [rsi]             # get low byte of 8-byte element`);
    lines.push(`    mov [rdi], al             # store in packed buffer`);
    lines.push(`    add rsi, 8                # next 8-byte element`);
    lines.push(`    inc rdi                   # next byte in packed buffer`);
    lines.push(`    dec rcx`);
    lines.push(`    jnz .write_bytes_pack_loop`);
    lines.push(``);
    lines.push(`.write_bytes_open:`);
    lines.push(`    # Open/create file (syscall 2: open)`);
    lines.push(`    mov rax, 2                # syscall: open`);
    lines.push(`    mov rdi, r15              # path pointer`);
    lines.push(`    mov rsi, 0x241            # flags: O_WRONLY|O_CREAT|O_TRUNC`);
    lines.push(`    mov rdx, 0x1A4            # mode: 0644 octal`);
    lines.push(`    syscall`);
    lines.push(``);
    lines.push(`    test rax, rax`);
    lines.push(`    js .write_bytes_error`);
    lines.push(`    mov r12, rax              # save fd`);
    lines.push(``);
    lines.push(`    # Write packed bytes (syscall 1: write)`);
    lines.push(`    mov rax, 1                # syscall: write`);
    lines.push(`    mov rdi, r12              # fd`);
    lines.push(`    mov rsi, r13              # packed buffer`);
    lines.push(`    mov rdx, r14              # byte count`);
    lines.push(`    syscall`);
    lines.push(``);
    lines.push(`    test rax, rax`);
    lines.push(`    js .write_bytes_close_error`);
    lines.push(``);
    lines.push(`    # Close file (syscall 3: close)`);
    lines.push(`    mov rax, 3`);
    lines.push(`    mov rdi, r12`);
    lines.push(`    syscall`);
    lines.push(``);
    ...
    lines.push(`    ret`);
    lines.push(``);
    lines.push(`.write_bytes_close_error:`);
    lines.push(`    mov rax, 3`);
    lines.push(`    mov rdi, r12`);
    lines.push(`    syscall`);
    lines.push(`.write_bytes_error:`);
    lines.push(`    xor rax, rax`);
    lines.push(`    pop rbx`);
    lines.push(`    pop r15`);
    lines.push(`    pop r14`);
